...
def choose_assets(canonical: dict[str, Any], save: dict[str, Any], team_abbrev: str, title: str, max_select: int | None = None) -> list[tuple[str, str]] | None:
    ...
    picks = sorted(
        [
            p for p in canonical.get("draft_picks", [])
            if p.get("current_owner_team_id") == team["id"]
            and p.get("id") not in used_picks
            and p.get("status") not in {"used_draft_pick", "expired_draft_pick"}
        ],
...
def apply_free_agency_offer(canonical: dict[str, Any], save_path: Path, offer_id: str, seed: int) -> dict[str, Any]:
    ...
    reserved = user_active_bid_commitment(save, team.get("id"), exclude_player_id=offer.get("player_id")) if offer.get("source") == "user" else 0.0
...
def print_trade_offer_details(canonical: dict[str, Any], candidate: dict[str, Any], save_path: Path | None = None) -> None:
    ...
    for team_id, assets in [
        ...
        for asset in assets:
            ...
            print(
                f"  {player.get('name', asset.get('label')):<23} age {age_text(player, 2)} {compact_position(player.get('position')):>3} "
